...
object TakeoverPolicy {
    fun prepare(call: Call, pending: PendingTakeover?): PendingTakeover {
        if (pending != null) {
            require(pending.callId == call.id) { "Resolve the pending takeover for call ${pending.callId} first." }
            return pending
        }
        return PendingTakeover(call.id, idempotencyKey = UUID.randomUUID().toString(), expectedCallVersion = call.version)
    }
